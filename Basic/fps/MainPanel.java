...
public class MainPanel extends JPanel implements Runnable {
  ...
  public void run() {
    ...
    while (running) {
      ...
      if (sleepTime > 0) {
        ...
        // 状態更新・レンダリングで時間を使いきってしまい休止時間がとれない場合
        overSleepTime = 0L;
        // 休止なしが16回以上続いたら
        if (++noDelays >= 16) {
          // 他のスレッドを強制実行
  ...
  private void gameRender() {
    if (dbImage == null) {
      ...
      if (dbImage == null) {
        System.out.println("dbImageが作成できない");
        return;
      } else {
